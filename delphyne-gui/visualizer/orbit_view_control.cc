...
/////////////////////////////////////////////////
OrbitViewControl::OrbitViewControl(const ignition::rendering::CameraPtr& _cam) : camera(_cam) {
...
/////////////////////////////////////////////////
void OrbitViewControl::OnMousePress(const QMouseEvent* _e) { this->OnMouseButtonAction(_e, ButtonState_t::PRESSED); }

/////////////////////////////////////////////////
void OrbitViewControl::OnMouseRelease(const QMouseEvent* _e) { this->OnMouseButtonAction(_e, ButtonState_t::RELEASED); }
...
//////////////////////////////////////////////////
const ignition::rendering::RayQueryResult& OrbitViewControl::GetQueryResult() const { return this->target; }

//////////////////////////////////////////////////
void OrbitViewControl::OnMouseButtonAction(const QMouseEvent* _e, const ButtonState_t& _state) {
  if (!this->camera) {
    return;
  }

  // Ignore unknown mouse buttons.
  if (_e->button() != Qt::LeftButton && _e->button() != Qt::RightButton && _e->button() != Qt::MidButton) {
    return;
  }
...
void OrbitViewControl::Update() {
  ...
  if (this->mouse.buttonDirty) {
    ...
    double nx = 2.0 * this->mouse.x / static_cast<double>(this->camera->ImageWidth()) - 1.0;
    double ny = 1.0 - 2.0 * this->mouse.y / static_cast<double>(this->camera->ImageHeight());
    this->rayQuery->SetFromCamera(this->camera, ignition::math::Vector2d(nx, ny));
    this->target = this->rayQuery->ClosestPoint();
    if (!this->target) {
      this->target.point = this->rayQuery->Origin() + this->rayQuery->Direction() * 10;
      return;
    }
  }
  ...
  if (this->mouse.motionDirty) {
    ...
    if (this->mouse.button == Qt::LeftButton && this->mouse.state == ButtonState_t::PRESSED) {
      this->viewControl.SetCamera(this->camera);
      this->viewControl.SetTarget(this->target.point);
      this->viewControl.Pan(drag);
    } else if (this->mouse.button == Qt::MidButton && this->mouse.state == ButtonState_t::PRESSED) {
      this->viewControl.SetCamera(this->camera);
      this->viewControl.SetTarget(this->target.point);
      this->viewControl.Orbit(drag);
    }
    // Right mouse button zoom.
    else if (this->mouse.button == Qt::RightButton && this->mouse.state == ButtonState_t::PRESSED) {
      double hfov = this->camera->HFOV().Radian();
      double vfov = 2.0f * atan(tan(hfov / 2.0f) / this->camera->AspectRatio());
      double distance = this->camera->WorldPosition().Distance(this->target.point);
      double amount =
          ((-this->mouse.dragY / static_cast<double>(this->camera->ImageHeight())) * distance * tan(vfov / 2.0) * 6.0);
